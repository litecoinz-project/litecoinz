...
UniValue getgenerate(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or litecoinz.conf setting gen)\n"
...
UniValue setgenerate(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "Generation is limited to 'genproclimit' processors, -1 is half available cores.\n"
            "See the getgenerate call for the current setting.\n"
            "\nArguments:\n"
            "1. generate         (boolean, required) Set to true to turn on generation, off to turn off.\n"
            "2. genproclimit     (numeric, optional) Set the processor limit for when generation is on. Can be -1 for half available cores.\n"
            "\nExamples:\n"
...
//            "  \"coinbasevalue\" : n,               (numeric) maximum allowable input to coinbase transaction, including the generation award and transaction fees (in Satoshis)\n"
...
#else
        throw JSONRPCError(RPC_METHOD_NOT_FOUND, "litecoinzd compiled without wallet and -mineraddress not set");
#endif
    }
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "LitecoinZ is not connected!");

    if (IsInitialBlockDownload(Params()))
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "LitecoinZ is downloading blocks...");
