...
BOOST_AUTO_TEST_CASE(rpc_wallet)
{
    ...
    BOOST_CHECK_EQUAL(find_value(obj, "miner").get_real(), 50);
    BOOST_CHECK_NO_THROW(retValue = CallRPC("getblocksubsidy 1000000"));
    obj = retValue.get_obj();
    BOOST_CHECK_EQUAL(find_value(obj, "miner").get_real(), 25);
...
BOOST_AUTO_TEST_CASE(rpc_z_mergetoaddress_parameters)
{
    ...
    CheckRPCThrows("z_mergetoaddress 1 2",
        "Error: z_mergetoaddress is disabled. Run './litecoinz-cli help z_mergetoaddress' for instructions on how to enable this feature.");
