...
WalletModel::WalletModel(const PlatformStyle *platformStyle, CWallet *wallet, OptionsModel *optionsModel, QObject *parent) :
    ...
    cachedNumBlocks(0),
    cachedTBalance(0), cachedZBalance(0), cachedUnshielded(0)
{
...
{
    // Z Balance
    CAmount zBalance = WalletModel::getZBalance(false);

    // T Balance
    CAmount tBalance = WalletModel::getTBalance(coinControl);

    return (zBalance + tBalance);
}

CAmount WalletModel::getUnshielded() const
{
    CAmount nBalance = 0;
    std::vector<COutput> vCoins;
...
{
    CAmount tUnconfirmed = wallet->GetUnconfirmedBalance();
    CAmount zUnconfirmed = (WalletModel::getZBalance(true) - WalletModel::getZBalance(false));
    return (tUnconfirmed + zUnconfirmed);
}
...
void WalletModel::checkBalanceChanged()
{
    ...
    CAmount newTBalance = getTBalance();
    CAmount newZBalance = getZBalance(false);
    CAmount newUnshielded = getUnshielded();
    ...
    if(cachedBalance != newBalance || cachedUnconfirmedBalance != newUnconfirmedBalance || cachedImmatureBalance != newImmatureBalance ||
        ...
        cachedTBalance != newTBalance || cachedZBalance != newZBalance || cachedUnshielded != newUnshielded)
    {
        cachedBalance = newBalance;
        cachedUnconfirmedBalance = newUnconfirmedBalance;
        cachedImmatureBalance = newImmatureBalance;

        cachedTBalance = newTBalance;
        cachedZBalance = newZBalance;
        cachedUnshielded = newUnshielded;
        ...
        Q_EMIT balanceChanged(newBalance, newUnconfirmedBalance, newImmatureBalance,
                            ...
                            newTBalance, newZBalance, newUnshielded);
    }
}
...
WalletModel::SendCoinsReturn WalletModel::prepareZTransaction(WalletModelTransaction &transaction, QString address, CAmount nBalance, CAmount nFee)
{
    CAmount total = 0;
...
{
    return coinSelectionTableModel;
}
