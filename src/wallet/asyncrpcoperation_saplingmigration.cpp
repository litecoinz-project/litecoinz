...
bool AsyncRPCOperation_saplingmigration::main_impl() {
    ...
    // If the remaining amount to be migrated is less than 0.01 LTZ, end the migration.
    ...
    do {
        ...
        for (const CSproutNotePlaintextEntry& sproutEntry : fromNotes) {
            ...
            // TODO: the above functionality (in comment) is not implemented in litecoinzd
            uint256 inputAnchor;
            std::vector<boost::optional<SproutWitness>> vInputWitnesses;
            pwalletMain->GetSproutNoteWitnesses(vOutPoints, vInputWitnesses, inputAnchor);
            builder.AddSproutInput(sproutSk, sproutNote, vInputWitnesses[0].get());
        }
        // The amount chosen *includes* the 0.0001 LTZ fee for this transaction, i.e.
        // the value of the Sapling output will be 0.0001 LTZ less.
        builder.SetFee(FEE);
